...
물론 더 정교한 시나리오에는 흐름이 필요합니다. 다만 "엑셀 = 무조건 흐름"이라는 통념은 **사실이 아닙니다.** 흐름 패턴은 다음 경우에 진가를 발휘합니다.

| 시나리오 | 질문 노드 단독 (실습 ①) | 흐름 + OS + AI 프롬프트 (실습 ②) |
|:--|:--|:--|
| 일회성 자연어 분석 | ✅ 충분 | 과도 |
| 행이 매우 많아 토큰 한계 초과 | ❌ | ✅ Office Script 사전 필터링 |
| 결과를 이메일·Teams로 송출 | ❌ | ✅ |
| 반복적·결정적 월간 리포트 | △ | ✅ |

다만 흐름 패턴에도 **함정**이 있습니다 — AI 프롬프트의 코드 인터프리터는 데이터 구조를 자세히 설명해주지 않으면 자주 실패하고, 모델·요청에 따라 끄는 게 오히려 답변이 깔끔한 경우도 있습니다. 이 부분은 실습 ②에서 정직하게 확인합니다.
...
같은 결과이되, 흐름 안에서 Office Script로 시트를 JSON으로 변환하고 AI 프롬프트가 코드 인터프리터로 계산. 큰 데이터·후속 자동화에 강합니다.
...
| 페이지 | 다루는 단계 | 한 줄 요약 |
|:---|:---|:---|
| [실습① — 질문 노드만으로 (메인)](s02-1-excel-question) | 토픽 + 질문 노드(파일) + A/B 체험 | **Table** 엑셀 / **raw** 엑셀 두 파일을 같은 토픽에 넣고 답변 품질 차이를 직접 확인 |
| [실습② — 흐름으로 디벨롭 (심화·선택)](s02-2-excel-flow-advanced) | 흐름 + Office Script + AI 프롬프트 | 흐름이 진짜 필요한 시나리오와 그 함정을 함께 체험 |

> **시간 배분 (30분)**: 도입 5 / 실습① 12 / 실습② 11 (시간 부족 시 데모 위주) / 마무리 2
...
| 파일 | 형식 | 용도 |
|:--|:--|:--|
| [과일판매.xlsx](../assets/files/과일판매.xlsx) | Excel **Table** 적용 (`FruitSales`) | 실습 ① "되는" 케이스 |
