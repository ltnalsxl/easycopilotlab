...
{: .warning }
> **이 한계 때문에 오전 [S2 — 엑셀 업로드 활용 실습](s02-excel-analysis) 가 만들어졌습니다.** 그쪽은 **토픽 + 질문 노드 + Excel Table** (메인) 또는 **흐름 + Office Script + AI 프롬프트** (심화) 로 우회하는 방법입니다. 즉, "사전 설정 없이 채팅에 Excel 을 그냥 던지면 AI 가 알아서 분석" 은 강의 내내 **시도하지 말 것** 으로 일관됩니다.
