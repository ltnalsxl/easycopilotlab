...
{: .highlight }
> 흐름에 URL을 직접 넣으면 환경이 바뀌는 순간 노동이 폭발합니다. 환경 변수를 쓰면 **값 하나만 바꾸면 끝**입니다. 여기서 직접 만들어보고, 이후 S6~S10에서 반복 활용합니다.
...
{: .important }
> **URL, 사이트 주소 등 환경마다 달라지는 값은 반드시 환경 변수로 빼세요.** 이후 실습(S6~S9)에서 이 변수들을 실제로 활용합니다.
