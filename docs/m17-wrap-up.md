...
| 2 | 파일 업로드 및 테스트 | M3 | ☐ |
| 3 | Copilot Studio로 가져오기 | M4 | ☐ |
| 4 | AI 모델 변경 + 설정 조정 | M6 | ☐ |
| 5 | 지침(행동매뉴얼) 작성 — 역할·범위·태도·원칙 | M7 | ☐ |
| 6 | 지식(교과서) 5개 파일 업로드 | M8 | ☐ |
| 7 | Echo Topic + Contact Topic 생성 | M10 | ☐ |
...
| 구성요소 | 비유 | 오늘 한 것 |
|:---------|:-----|:----------|
| **오케스트레이터** | 엔진 | 모델 선택, 설정 조정 (M6) |
| **지침** | 행동매뉴얼 | 역할·범위·태도·원칙 작성 (M7) |
| **지식** | 교과서 | 5개 문서 업로드 (M8) |
| **도구** | 손발 | Topic, 커넥터, 흐름, 트리거 (M9~M16) |
...
| 증상 | 원인 | 해결 |
|:-----|:-----|:-----|
| 에이전트가 엉뚚한 답을 한다 | 지식이 없거나 부족 | M8 — 관련 문서 추가 업로드 |
| 범위 밖 질문에도 답한다 | 지침에 범위가 불명확 | M7 — 범위를 "~만 답변"으로 수정 |
| 모르는 것을 지어낸다 | "모를 때 행동" 미정의 | M7 — 원칙에 모름 메시지 추가 |
