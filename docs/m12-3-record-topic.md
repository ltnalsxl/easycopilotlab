...
![토픽 목록 — "+ 토픽 추가" → "새로 시작" 선택](../assets/images/m12/image6.png)

Topic 이름을 `Record Topic`으로 입력하고, 트리거 노드의 **"트리거 변경"** 아이콘을 클릭합니다. 트리거 유형 목록에서 **"AI가 생성한 응답이 곧 전송됩니다"**를 선택합니다.

![트리거 변경 — "AI가 생성한 응답이 곧 전송됩니다" 선택](../assets/images/m12/image7.png)

트리거가 **"생성된 AI 응답"**으로 변경된 것을 확인합니다.

![트리거 설정 완료 — "생성된 AI 응답" 트리거](../assets/images/m12/image8.png)
...
!["+" 메뉴 → "도구 추가" 선택](../assets/images/m12/image9.png)

도구 추가 패널의 **커넥터** 탭에서 "테이블에 행"을 검색하고, **"테이블에 행 추가 — Excel Online (Business)"**를 선택합니다.

![도구 추가 — 커넥터 탭에서 "테이블에 행 추가" 검색 및 선택](../assets/images/m12/image10.png)

연결 만들기 화면에서 Excel Online(Business) 연결이 ✅ 확인되면 **"제출"**을 클릭합니다.

![연결 만들기 — Excel Online(Business) 연결 확인 후 "제출"](../assets/images/m12/image11.png)

커넥터가 추가되면 "Add a row into a table" 노드가 생성됩니다.

![커넥터 추가 완료 — "Add a row into a table" 노드 생성](../assets/images/m12/image12.png)
...
| 표 | `표1` 선택 |
   ...
   ![입력 패널 — Location, Document Library, File, Table, Row 설정 필드](../assets/images/m12/image13.png)

   각 항목을 아래와 같이 설정합니다.

   ![입력 설정 — OneDrive for Business, 대화기록.xlsx, 표1 선택 완료](../assets/images/m12/image14.png)
   ...
   ```
{
   ...
   ![수식 입력 — 시간: Now(), 사용자: System.User.PrincipalName, 질문: System.Activity.Text 입력](../assets/images/m12/image15.png)

   Row에 수식이 설정된 것을 확인합니다.

   ![Row에 수식 설정 완료 확인](../assets/images/m12/image16.png)

11. **속성** 클릭

   노드의 **"..."** 메뉴를 클릭하고 **"속성"**을 선택합니다.

   ![노드 "..." 메뉴 → "속성" 선택](../assets/images/m12/image17.png)

   **개시** 탭에서 최종 사용자 인증을 **"제작자 제공 자격 증명"**으로 설정하고 **저장**합니다.

   ![속성 — 개시 탭, 최종 사용자 인증: "제작자 제공 자격 증명" 설정 후 저장](../assets/images/m12/image18.png)
...
> Excel에 시간·사용자·질문·답변이 쌓이는 걸 확인하면 성공입니다.
   ...
   ![테스트 — "연차 며칠이야?" 질문에 답변 + Record Topic 실행](../assets/images/m12/image19.png)

   OneDrive에서 대화기록.xlsx를 열면 시간·사용자·질문·답변이 기록되어 있습니다!

   ![Excel — 대화기록.xlsx에 새 행 추가 확인 (시간, 사용자, 질문, 답변)](../assets/images/m12/image20.png)
