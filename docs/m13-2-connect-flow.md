...
![에이전트 흐름 목록에서 flow_HRRequest가 게시됨 상태로 표시된 화면](../assets/images/m13/image31.png)
...
![HR 도우미 에이전트 도구 탭에서 + 도구 추가 버튼을 클릭하는 화면](../assets/images/m13/image32.png)

**② "흐름" 탭 선택 → flow_HRRequest 클릭**

![도구 추가 팝업에서 흐름 탭을 선택하고 flow_HRRequest를 클릭하는 화면](../assets/images/m13/image33.png)

**③ "추가 및 구성" 클릭**

![flow_HRRequest 선택 후 추가 및 구성 버튼을 클릭하는 화면](../assets/images/m13/image34.png)

**④ 도구 세부 정보 확인**

이름, 설명, 에이전트 흐름 연결, 입력 파라미터가 올바르게 표시되는지 확인합니다.

![flow_HRRequest 도구 세부 정보 화면 — 이름, 설명, 입력 항목이 표시됨](../assets/images/m13/image35.png)

**④-1 mySender 입력을 시스템 변수로 변경**

mySender 항목의 "AI로 동적으로 채우기" 옆 **"사용자 지정"**을 클릭합니다.

![mySender 입력 항목에서 사용자 지정을 클릭하는 화면](../assets/images/m13/image35-1.png)

**④-2 시스템 변수 선택**

변수 선택 패널에서 **시스템** 탭 → "user" 검색 → **User.DisplayName (System.User.DisplayName)**을 선택합니다.

![변수 선택 패널에서 시스템 탭의 User.DisplayName을 선택하는 화면](../assets/images/m13/image35-2.png)

**④-3 변수 지정 완료 확인**

mySender 값에 **User.DisplayName** 변수가 지정된 것을 확인하고 **저장**합니다.

![mySender에 User.DisplayName 변수가 지정된 완료 화면](../assets/images/m13/image35-3.png)

**⑤ 도구 목록에서 등록 확인**

도구 탭으로 돌아오면 **flow_HRRequest**가 흐름 유형으로 등록된 것을 확인할 수 있습니다.

![도구 탭에서 flow_HRRequest가 흐름으로 등록된 목록 화면](../assets/images/m13/image36.png)
...
![지침 편집 화면에서 STRICT RULES에 flow_HRRequest 흐름 호출 규칙이 추가된 모습](../assets/images/m13/image37.png)
...
![테스트 패널에서 flow_HRRequest 실행 중 Office 365 Outlook 연결 허용 팝업이 표시된 화면](../assets/images/m13/image38.png)

**② 흐름 실행 완료 확인**

흐름이 정상 실행되면 담당자 메일로 문의 내용이 전달되고, 에이전트가 완료 메시지를 안내합니다.

![테스트 완료 — 흐름 실행 결과와 성공 메시지가 표시된 화면](../assets/images/m13/image39.png)

**③ Outlook에서 메일 수신 확인**

담당자 메일함(hr@abc.co.kr)에서 실제로 문의 메일이 도착했는지 확인합니다.

![Outlook 보낸 편지함에서 flow_HRRequest로 발송된 문의 메일을 확인하는 화면](../assets/images/m13/image40.png)

| # | 질문 | 기대 동작 |
|:--|:-----|:---------|
| 1 | "복지포인트 문의를 담당자에게 보내줘" | flow_HRRequest 호출 → 메일 발송 → 완료 안내 |
| 2 | "경비처리 방법 알려줘" | 지식 검색 또는 Echo Topic 호출 (기존 지침대로) |
