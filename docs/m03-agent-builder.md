...
2. 에이전트 빌더에서 **파일을 업로드**하면 업로드 문서 기반 답변이 가능
3. **만들기 버튼**으로 배포하면 몰입형과 인컨텍스트 두 방식으로 사용 가능
4. Copilot Studio가 필요한 이유는 **두 가지** — 더 다양한 지식 소스, 더 다양한 도구
5. 다음 M4에서 에이전트를 **Copilot Studio로 복사**하여 고급 기능을 사용할 준비를 합니다
