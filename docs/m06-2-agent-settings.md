...
![에이전트 개요 화면 — 상단 우측 ⚙️ 설정 버튼 위치](../assets/images/m06/image2.png)
...
| 2 | **연결된 에이전트** | **켜기** (확인) | M15 멀티에이전트 실습에서 다른 에이전트와 협업하려면 켜져 있어야 합니다. |
...
| 8 | **작업 IQ 켜기** | **끄기** | 지식문서가 쉐어포인트에 저장되어 있을 때, 에이전트가 이를 인식하고 활용할 수 있는 기능입니다. 본 실습에서는 필요하지 않으므로 끕니다. |
...
![설정 — 오케스트레이션(예) 및 연결된 에이전트(켜기)](../assets/images/m06/image3.png)

**② 응답 형식 지정 (그대로 유지)**

![설정 — 응답 형식 지정](../assets/images/m06/image4.png)

**③ 조정(낮음) · 사용자 피드백**

![설정 — 조정(낮음) 및 사용자 피드백](../assets/images/m06/image5.png)

**④ 사용자 피드백(끄기)**

![설정 — 사용자 피드백(끄기)](../assets/images/m06/image6.png)

**⑤ 파일 업로드(끄기) · 코드 인터프리터(끄기) · 작업 IQ(켜기) → 저장**

![설정 — 파일 업로드(끄기), 코드 인터프리터(끄기), 작업 IQ(켜기) 및 저장 버튼](../assets/images/m06/image7.png)
...
![설정 저장 완료 — "변경 사항이 저장되었습니다" 메시지 확인 후 ✕ 닫기](../assets/images/m06/image8.png)
...
| 연결된 에이전트 | 켜기 | M15 멀티에이전트 |
| 조정 | 낮음 | 전체 (필터 완화) |
| 작업 IQ | 켜기 | M13 에이전트 흐름 |
| 사용자 피드백 | 끄기 | — |
| 파일 업로드 | 끄기 | — |
| 코드 인터프리터 | 끄기 | — |
...
실습을 완료했으면 [M6 본문으로 돌아가세요](m06-orchestrator).
