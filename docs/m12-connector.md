...
![M12 커넥터 — 자동으로 일기 쓰는 AI](../assets/images/m12/hero.png)
...
커넥터가 어떻게 동작하는지 먼저 간단하게 체험합니다. MSN 날씨 커넥터를 추가하여 에이전트가 날씨 정보를 직접 가져오는 것을 확인합니다.

{: .important }
> 📌 이 실습은 별도 페이지에서 진행합니다.  
> [실습 ①: MSN 날씨 커넥터 체험](m12-1-msn-weather)를 완료하고 돌아오세요.
...
> 방금 한 것은 Power Automate 흐름을 만든 것이 아니라, **Topic 안에서 Excel 커넥터를 바로 호출해 한 줄을 저장한 것**입니다. 즉, 앱 하나에 단일 동작을 바로 붙여본 것입니다. 다음 M13에서는 여기서 한 단계 더 나아가, 정보를 수집하고 AI로 문안을 만들고 메시지까지 보내는 **여러 단계 자동화 흐름**으로 확장하겠습니다.
...
4. M13에서는 이 단일 동작을 넘어, **여러 단계를 묶는 Agent Flow**로 확장합니다
...
다음 모듈: [M13. 도구 — 에이전트 흐름](m13-agent-flow)
