...
## S1. 문서 업로드 활용 실습 — 도입부 4컷 만화

**파일명:** `assets/images/s01/01_세션1_4컷만화_PDF진화.png`

```
한국 직장인 대상 Copilot Studio 교육 자료 세션 1 도입에 들어갈 4컷 만화 일러스트를 그려줘.

스토리: "각양각색 PDF가 매일 쌓인다. 사람이 표준 양식 Word로 옮기느라 야근. AI 에이전트가 대신 해주니 사람은 검토만."

스타일:
- 깔끔한 라인아트 + 부드러운 파스텔 컬러
- 액센트 컬러: 네이비 (#1F3864)
- 직장인 캐릭터 한 명 (남녀 무관)
- 2x2 그리드, 정사각형, 각 컷 위에 한국어 캡션 (또렷)
- 비즈니스 친화적, 살짝 유머

4컷:
1컷 (캡션: "매일 쌓이는 제각각 양식의 PDF"): 책상 위에 회의록·계약서·견적서·평가서 PDF가 산처럼 쌓여 있고, 직장인은 한숨을 쉰다.

2컷 (캡션: "회사 표준 양식으로 옮기는 단순 반복 작업"): 화면에 회사 로고가 박힌 표준 Word 양식을 띄워놓고, 직장인이 PDF를 보며 손으로 복사·붙여넣기 중. 표정은 지쳐 있다.

3컷 (캡션: "또 야근…"): 벽시계는 밤 9시. 사무실은 어둡고, 직장인 책상만 불이 켜져 있다. 머리를 부여잡는다.

4컷 (캡션: "추출·변환은 에이전트가, 사람은 검토만"): 옆에 친근한 AI 에이전트(작은 로봇/캐릭터)가 PDF를 받아 자동으로 표준 Word로 변환해 건네준다. 직장인은 한 손에 커피, 환하게 웃는 표정.

가로 1024 x 세로 1024, 한국어 텍스트가 또렷하게.
```
